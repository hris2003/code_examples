...
class DroneVideoDisplay(Thread):
	StatusMessages = {
	...
	def __init__(self, folder):
		...
		#self.title('AR.Drone Video Feed')
		#self.initGUI();
	...
	def start(self):
		...
		self.subVideo   = rospy.Subscriber('/image_raw',Image,self.ReceiveImage,queue_size=1, buff_size=2**24)
		Thread.start(self)
    	def run(self):
	...
	def initGUI(self):
		r =0
        	self.quitBtn = Tkinter.Button(self, text="Quit", command=self.quit)
        	self.quitBtn.grid(column=0, row = r, columnspan=2)
	
	def quit(self):
		self.subNavdata.unregister();
		self.subVideo.unregister();
